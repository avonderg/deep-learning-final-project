...
def train_lstm(model, input_data, target_data, learning_rate=0.001, num_epochs=100):
    ...
    for epoch in range(num_epochs):
        ...
        output = output.view(-1, model.hidden2tag.out_features)  # (batch_size * sequence_length, tagset_size)
        target = target_data.view(-1)  # (batch_size * sequence_length)
...
def main():
    ...
    trainX_tensor = torch.tensor(trainX).float().unsqueeze(-1)  # (batch_size, sequence_length, input_dim)
    trainY_tensor = torch.tensor(trainY).float()  # (batch_size, output_dim)
    testX_tensor = torch.tensor(testX).float().unsqueeze(-1)  # (batch_size, sequence_length, input_dim)
    testY_tensor = torch.tensor(testY).float()  # (batch_size, output_dim)

    # Create an instance of the LSTMTagger model
    embedding_dim = 100
    hidden_dim = 128
    vocab_size = 5000
    tagset_size = 10

    model = LSTM(embedding_dim, hidden_dim, vocab_size, tagset_size)

    # Train the model
    train_lstm(model, trainX_tensor, trainY_tensor, learning_rate=0.001, num_epochs=200)
